...
const SEGMENT_RADIUS = {
  spine: 0.10, chest: 0.105, neck: 0.05, head: 0.095,
  shoulder_l: 0.055, elbow_l: 0.048, wrist_l: 0.038,
  shoulder_r: 0.055, elbow_r: 0.048, wrist_r: 0.038,
  hip_l: 0.075, knee_l: 0.072, ankle_l: 0.052,
  hip_r: 0.075, knee_r: 0.072, ankle_r: 0.052,
};
...
function Mannequin3D({ poseKey, accent, size = 220 }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Two cameras sharing one scene: front (0°) and side (90°) — a single angle
      // hides exactly the motion that matters for hinge/squat/lunge-type movements.
      const makeCamera = () => {
        const cam = new THREE.PerspectiveCamera(30, 1, 0.1, 20);
        cam.position.set(0, 1.05, 3.3);
        cam.lookAt(0, 0.82, 0);
        return cam;
      };
      const frontCam = makeCamera();
      const sideCam = makeCamera();
      sideCam.position.set(3.3, 1.05, 0);
      sideCam.lookAt(0, 0.82, 0);
      ...
      const floorMat = new THREE.MeshStandardMaterial({ color: 0x000000, transparent: true, opacity: 0.14 });
      const floor = new THREE.Mesh(new THREE.CircleGeometry(1.35, 40), floorMat);
    ...
    return () => {
      ...
      if (stateRef.current) {
        ...
        stateRef.current.scene.traverse((obj) => { if (obj.material) obj.material.dispose(); });
        stateRef.current.geoms.forEach((g) => g.dispose());
