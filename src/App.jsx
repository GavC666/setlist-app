...
const MOVE_DATA = {
  ...
  pushup:   { hold: 0.5, move: 1.1, a: pose(-84, -0.60, { shoulder_l: j(90, 0, -6), shoulder_r: j(90, 0, 6), elbow_l: j(-8), elbow_r: j(-8) }), b: pose(-84, -0.72, { shoulder_l: j(78, 0, -22), shoulder_r: j(78, 0, 22), elbow_l: j(-78), elbow_r: j(-78) }) },
...
function LibraryTab({ onAdd, accent }) {
  ...
  return (
    <div className="tab-pane">
      ...
      <div className="ex-list">
        {filtered.slice(0, 200).map((ex) => (
          <button key={ex.id} className="ex-row" onClick={() => setDetail(ex)}>
            ...
            {onAdd && <span className="ex-row-add" onClick={(e) => { e.stopPropagation(); handleAdd(ex); }}><Plus size={18} /></span>}
          </button>
        ))}
      ...
      <ExerciseSheet ex={detail} onClose={() => setDetail(null)} onAdd={onAdd ? handleAdd : null} accent={accent} />
